...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v9.0", layout="wide")
...
st.title("VRC / HRV RRi Analyzer Pro v9.0")
...
#
# El script busca HTML en:
#   - graficos/html/*.html  (estructura del ZIP exportado por la app)
#   - *.html                (si lo colocas junto a los HTML)
#
# Crea:
#   - graficos/png_from_html/*.png
#   - graficos_png_desde_html.zip
...
HTML_DIRS = [BASE / "graficos" / "html", BASE]
OUT_DIR = BASE / "graficos" / "png_from_html"
OUT_DIR.mkdir(parents=True, exist_ok=True)
ZIP_OUT = BASE / "graficos_png_desde_html.zip"
...
async def main():
    ...
    # Evitar duplicados si BASE y graficos/html coinciden por enlaces/rutas
    seen = set()
    html_files_unique = []
    for h in html_files:
        rp = h.resolve()
        if rp not in seen:
            seen.add(rp)
            html_files_unique.append(h)
    html_files = html_files_unique

    if not html_files:
        print("No se han encontrado archivos HTML.")
        print(f"Buscado en: {[str(d) for d in HTML_DIRS]}")
        return
    ...
    async with async_playwright() as p:
        ...
        for html in html_files:
            ...
            # Captura de la figura completa, no solo la ventana visible.
...
def _ensure_kaleido_chrome():
    """
    Asegura Chrome/Chromium para Kaleido cuando sea posible.

    Importante:
    - En local puede descargar/activar Chrome con pio.get_chrome().
    - En Streamlit Cloud es más estable añadir un archivo packages.txt con: chromium
    - Si no puede instalarse, la app no se rompe; se exporta HTML y se deja aviso.
    """
    try:
        # Plotly >= 6 / Kaleido >= 1 puede necesitar Chrome externo.
        # Si Chrome ya existe, esto no hace daño. Si falta, intenta obtenerlo.
...
def _try_write_plotly_png(fig, out_path, width=1800, height=1100, scale=2):
    """
    Guarda una figura Plotly como PNG.

    1) Intenta exportación directa con Kaleido.
    2) Si falla por falta de Chrome/Chromium, intenta preparar Chrome.
    3) Reintenta exportar.

    Devuelve:
    - ok: bool
    - error: texto del error si no se pudo exportar
    """
    try:
        fig.write_image(str(out_path), format="png", width=width, height=height, scale=scale)
        return True, ""
    except Exception as e1:
        ok_chrome, chrome_msg = _ensure_kaleido_chrome()
        try:
            fig.write_image(str(out_path), format="png", width=width, height=height, scale=scale)
            return True, ""
        except Exception as e2:
            msg = (
                "No se pudo exportar PNG con Kaleido. "
                f"Primer error: {e1}. "
                f"Chrome/Kaleido: {chrome_msg}. "
                f"Segundo error: {e2}."
            )
            return False, msg
...
def write_all_graph_exports(figures, outdir, formats=("html",)):
    ...
    for i, (name, fig) in enumerate(figures, start=1):
        ...
        if "png" in formats:
            ...
            ok_png, png_error = _try_write_plotly_png(fig, png_path)
            if ok_png:
                png_name = f"png/{png_path.name}"
            else:
                png_errors.append((base, png_error))
    ...
    if png_errors:
        (outdir / "AVISO_PNG.txt").write_text(
            ...
            + "Detalle de errores:\n"
            + "\n".join([f"- {name}: {err[:500]}" for name, err in png_errors]),
            encoding="utf-8"
        )
    ...
    if long_df.empty:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            export_formats = st.multiselect(
                "Formatos de gráficos a incluir en el ZIP",
            ...
            # Archivos de ayuda para que PNG funcione en Streamlit Cloud con Kaleido.
            # En Streamlit Cloud, además de kaleido en requirements.txt, suele hacer falta packages.txt con chromium.
            p_requirements_png = tmpdir / "requirements_png.txt"
            p_packages_png = tmpdir / "packages.txt"
            p_requirements_png.write_text(
                "streamlit\n"
                "numpy\n"
                "pandas\n"
                "scipy\n"
            ...
            p_packages_png.write_text("chromium\n", encoding="utf-8")

            st.caption(
                f"Se han preparado {len(figures_to_export)} gráficos. "
                "Si PNG falla en Streamlit Cloud, usa los HTML y el script convert_html_to_png.py incluido en el ZIP."
            )
            ...
            with zipfile.ZipFile(zipf, "w", zipfile.ZIP_DEFLATED) as z:
                ...
                # Añadir archivos auxiliares para desplegar con PNG en Streamlit Cloud
                if p_requirements_png.exists():
                    z.write(p_requirements_png, arcname=p_requirements_png.name)
                if p_packages_png.exists():
                    z.write(p_packages_png, arcname=p_packages_png.name)
