...
# Paleta explícita para que HTML y PNG exportados salgan siempre a color.
# Sin esto, algunas figuras de Plotly pueden salir en gris/negro al exportarse.
RECORD_COLORS = [
    "#0096ff",  # azul
    "#ff8c00",  # naranja
    "#00c864",  # verde
...
def _phase_color(phase):
    grupo = PHASE_GROUP.get(phase, phase)
    return PHASE_LINE_COLORS.get(grupo, "#0096ff")

def _record_color(index):
    return RECORD_COLORS[int(index) % len(RECORD_COLORS)]
...
def hvg_network_figure(rr, title="HVG", max_nodes=140):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(width=0.7, color="rgba(80,80,80,0.35)"),
        hoverinfo="skip",
        showlegend=False
    ))
    fig.add_trace(go.Scatter(
        ...
        fig.add_trace(
            go.Scatter(
                ...
                marker=dict(
                    ...
                    opacity=0.72,
                    color=color,
                    line=dict(width=0.3, color="rgba(0,0,0,0.30)")
                ),
...
def hvg_network_compare_figure(record_data, global_windows, record_windows, phase, use_independent, max_nodes=120):
    ...
    for idx, rec in enumerate(records):
        ...
        fig.add_trace(
            go.Scatter(
                x=edge_x, y=edge_y, mode="lines",
                line=dict(width=0.55, color="rgba(80,80,80,0.35)"),
                hoverinfo="skip",
                showlegend=False
            ),
            row=r, col=c
        ...
        fig.add_trace(
            go.Scatter(
                x=node_x, y=node_y, mode="markers",
                marker=dict(size=node_size, color=_record_color(idx), opacity=0.86, line=dict(width=0.4, color="rgba(0,0,0,0.35)")),
                text=node_text,
                hoverinfo="text",
                showlegend=False
            ),
...
def poincare_figure(record_data, global_windows, record_windows, phase, use_independent):
    """
    Poincaré comparativo superpuesto.

    Colores:
    - Si hay varios registros, cada registro usa un color propio.
    - Si sólo hay un registro, usa el color de la fase:
      Basal azul, Ejercicio naranja, Recuperación verde.
    """
    fig = go.Figure()
    records = list(record_data.items())
    n_records = len(records)

    for idx, (rec, data) in enumerate(records):
        windows = get_record_windows(global_windows, record_windows, rec, use_independent)
        w = windows.get(phase)
        if w is None:
            continue
        ...
        color = _phase_color(phase) if n_records <= 1 else _record_color(idx)
